...
with tab_predict:
    ...
    st.caption("ใส่ข้อมูลของนักศึกษาเพื่อให้ AI พยากรณ์ค่าใช้จ่ายรายเดือน")
    feature_labels = {
    "gender": "เพศ",
    "age": "อายุ (ปี)",
    "year_in_school": "ชั้นปีการศึกษา",
    "major": "สาขาวิชา",
    "scholarship": "ทุนการศึกษา (มี/ไม่มี)",
    "financial_aid": "เงินสนับสนุน/เงินช่วยเหลือ (บาท)",
    "monthly_income": "รายได้ต่อเดือน (บาท)",
    "tuition": "ค่าเทอม (บาท)",
    "housing": "ค่าที่พัก (บาท)",
    "food": "ค่าอาหาร (บาท)",
    "transportation": "ค่าเดินทาง (บาท)",
    "books_supplies": "ค่าอุปกรณ์/หนังสือเรียน (บาท)",
    "entertainment": "ค่าใช้จ่ายด้านบันเทิง (บาท)",
    "technology": "ค่าเทคโนโลยี เช่น มือถือ อินเทอร์เน็ต (บาท)",
    "personal_care": "ค่าดูแลส่วนบุคคล (บาท)",
    "health_wellness": "ค่ารักษาพยาบาล/สุขภาพ (บาท)",
    "miscellaneous": "ค่าใช้จ่ายอื่น ๆ (บาท)",
    "games_hobbies": "ค่าเกม/งานอดิเรก",
    "cosmetics_selfcare": "ค่าเครื่องสำอาง/ดูแลตัวเอง",
    "transporting": "รูปแบบการเดินทาง (เช่น รถยนต์, มอเตอร์ไซค์)"
    }

    # 🧩 สร้าง dictionary สำหรับเก็บค่าที่ผู้ใช้กรอก
    user_inputs = {}
    ...
    for i, col in enumerate(features_selected):
        series = df[col]
        display_name = feature_labels.get(col, col)  # ถ้าไม่มีในพจนานุกรมจะใช้ชื่อเดิม

        if series.dtype.kind in "biufc":  # 🔢 ฟีเจอร์ตัวเลข
            ...
            with target_col_ui:
                user_inputs[col] = st.number_input(
                    ...
                    help=f"ช่วงค่าที่เคยพบในข้อมูล: {v_min:.2f} – {v_max:.2f} (สามารถกรอกค่าอื่นได้)"
            )
        else:  # 🔠 ฟีเจอร์หมวดหมู่
    ...
    # =============== ปุ่มทำนาย (อยู่ในบล็อกเดียวกัน) ===============
    if st.button("ทำนายเลย 🔮"):
        X_one = pd.DataFrame([user_inputs], columns=features_selected)
        pred_class = int(final_pipe.predict(X_one)[0])

        prob = None
        if hasattr(final_pipe.named_steps["clf"], "predict_proba"):
            prob = float(final_pipe.predict_proba(X_one)[:, 1][0])

        # แสดงผลการ์ดสีแดง/เขียว (ตามที่พี่ให้ก่อนหน้า)
        if pred_class == 1:
        # ใช้จ่ายสูง (สีแดง)
            st.markdown(f"""
            <div class="pred-card pred-red">
            <h3>🔺 นักศึกษาคนนี้มีแนวโน้ม <u>ใช้จ่ายสูงกว่าค่าเฉลี่ย</u></h3>
            <p class="pred-meta">
            ระบบจัดอยู่ในกลุ่ม “ใช้จ่ายสูง (Label = 1)” เนื่องจากค่าใช้จ่ายคาดการณ์ ≥ ค่าเกณฑ์ (threshold) ที่กำหนด
            {f"<br>ความมั่นใจของโมเดล ≈ <b>{prob:.2f}</b> ({prob*100:.1f}%)" if prob is not None else ""}
            </p>
            <ul class="pred-meta">
                <li>คำแนะนำเบื้องต้น: ตรวจสอบหมวดค่าใช้จ่ายที่สูง เช่น ค่าอาหาร/เดินทาง/บันเทิง และตั้งงบประมาณรายเดือน</li>
                <li>สามารถปรับ threshold ในหน้า “ข้อมูล” เพื่อดูขอบเขตอื่น ๆ ได้</li>
            </ul>
            </div>
        """, unsafe_allow_html=True)
        else:
        # ไม่สูง (สีเขียว)
            st.markdown(f"""
            <div class="pred-card pred-green">
            <h3>🟢 นักศึกษาคนนี้มีแนวโน้ม <u>ใช้จ่ายปกติ</u></h3>
            <p class="pred-meta">
            ระบบจัดอยู่ในกลุ่ม “ใช้จ่ายปกติ (Label = 0)” เนื่องจากค่าใช้จ่ายคาดการณ์ต่ำกว่าเกณฑ์ (threshold)
            {f"<br>ความมั่นใจของโมเดล ≈ <b>{prob:.2f}</b> ({prob*100:.1f}%)" if prob is not None else ""}
            </p>
            <ul class="pred-meta">
                <li>ข้อแนะนำ: รักษาวินัยการใช้จ่ายเดิม และสำรองเงินฉุกเฉิน 3–6 เท่าของรายจ่ายรายเดือน</li>
            </ul>
            </div>
        """, unsafe_allow_html=True)

        # บันทึกไฟล์ CSV
        out_row = {**user_inputs, "pred_label": pred_class}
        if prob is not None:
            out_row["pred_prob_high"] = round(prob, 4)

        export_one = pd.DataFrame([out_row])
        st.download_button(
            "⬇️ ดาวน์โหลดผลทำนายแถวนี้ (CSV)",
            data=export_one.to_csv(index=False).encode("utf-8"),
